...
| 选词填空 | 10 题 |
| 词句匹配 | 至少 5 组；每组保持 4 个问句 + 4 个唯一对应回答，不得只用总配对数凑数 |
| 词了个词 | 至少 5 组；每组建议 4 个词/短语/句子配对，覆盖词义、短语和句子理解 |
| 汉字辨析 | 10 题，选项不能泄题，汉字选项默认不显示拼音 |
| 句子转换 | 10 题，必须体现句型转换，不得全部退化为选词填空；拼音必须显示在汉字上方 |
...
后续新课数据生成与验收时，除遵守本契约字段结构外，还必须同步检查 `docs/HSK1课程数据质量参考.md`。该文档沉淀 L01-L05 反复测试中出现的课前、课中、课后、拼音、汉字认读、批改链路和数据记录问题，用于在数据生成阶段提前避免同类问题。
