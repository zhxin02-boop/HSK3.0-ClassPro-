...
DATA_PATH = ROOT / "source" / "data-model" / "mock-tests" / "HSK1-mock-01.json"
OUT_DIR = ROOT / "docs" / "mock-test-print"
DOCX_PATH = OUT_DIR / "HSK1-mock-01-print-teacher-pack.docx"
SCRIPT_PATH = OUT_DIR / "HSK1-mock-01-listening-script.txt"
ANSWER_PATH = OUT_DIR / "HSK1-mock-01-answer-key.txt"
...
def add_student_item(doc, item):
    ...
    r.font.name = "Microsoft YaHei"
    r.font.size = Pt(10.5)
    question = item.get("question") or "（听录音作答）"
...
def write_text_files(test):
    script = ["HSK1 模拟测试 01 - 听力朗读稿", "每题读两遍；题间停顿 4-5 秒。", ""]
    script.extend(listening_lines(test))
    SCRIPT_PATH.write_text("\n".join(script) + "\n", encoding="utf-8")

    answers = ["HSK1 模拟测试 01 - 正确答案", ""]
